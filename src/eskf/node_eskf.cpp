...
void odom_callback_vo(const nav_msgs::Odometry::ConstPtr &msg)
{
  ...
  else
  {
    ...
    {
      //use the first vo input at the init value
      eskf_imu->init_from_vo(msg->header.stamp.toSec(),//将时间戳转化为浮点数形式
...
{
  //ESKF作为滤波的部分，会需要经常进行更新和输入数据，输入的数据则是IMU的部分，通过IMU的信息对数据进行滤波。
  ros::init(argc, argv, "eskf_node");//eskf_node
  ros::NodeHandle n("~");


  odom_pub = n.advertise<nav_msgs::Odometry>("eskf_odom", 10);

  // You should also tune these parameters，参数可能需要进行调整
  ...
  n.getParam("eskf/vo_delay_ms", vo_delay);//初始化的时候设置为0了
  /* optical flow noise */
  ...
  ros::Subscriber s1 = n.subscribe("imu", 30, imu_callback);//IMU初始化
  ros::Subscriber s2 = n.subscribe("vo", 1, odom_callback_vo);//订阅之后的callback函数运行
